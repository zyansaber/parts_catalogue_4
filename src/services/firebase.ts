...
const DEFAULT_R2_ENDPOINT = 'https://c2fc5b05a19391736a9dceef43b0d545.r2.cloudflarestorage.com';
const DEFAULT_R2_ACCESS_KEY = '60c5f55b3c813d956e1c3a90cf6893d8';
const DEFAULT_R2_SECRET_KEY = '16ca6a9e8ed4a55b6b02cff6f1c2bf3982c59910aa41aeec6e2e308ec76990b2';
...
export class FirebaseService {
  private static getR2Config() {
    ...
    const secretAccessKey = (import.meta.env.VITE_R2_SECRET_KEY || DEFAULT_R2_SECRET_KEY).trim();
    const bucket = (import.meta.env.VITE_R2_BUCKET || DEFAULT_R2_BUCKET).trim();
  ...
  static async savePartApplication(application: any): Promise<void> {
    try {
      await set(ref(database, `partApplications/${application.id}`), {
        ...
        minimumOrderQuantity: application.minimumOrderQuantity || '',
        changeField: application.changeField || '',
        changeFields: application.changeFields || [],
        priceChangeDirection: application.priceChangeDirection || '',
        isNewSupplier: Boolean(application.isNewSupplier),
        ...
        isPack: Boolean(application.isPack),
        packQuantity: application.isPack ? application.packQuantity || '' : '',
  ...
  static async approvePartApplication(applicationId: string, partCode: string, replacementImageUrl = ''): Promise<string> {
    try {
      ...
      await set(ref(database, `material_summary_2025/${partCode}`), {
        ...
        Is_Pack: Boolean(currentData.isPack),
        Pack_Quantity: currentData.isPack ? currentData.packQuantity || '' : '',
