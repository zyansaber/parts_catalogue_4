...
export default function OpenPoVendor3060Page() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      get(ref(database, 'production_report/open_po_extra_by_po')),
  ...
  useEffect(() => {
    const initExtras = async () => {
      ...
      await Promise.all(
        Object.entries(updatesByPo).map(([po, patch]) => update(ref(database, `production_report/open_po_extra_by_po/${po}`), patch)),
      );
    };
  ...
  const updateExtraField = async (poNumber: string, field: keyof OpenPoExtraFields, value: string) => {
    ...
    await update(ref(database, `production_report/open_po_extra_by_po/${poNumber}`), {
      [field]: value,
    });
  };
