...
const getAgeDays = (ticketEntry: Ticket) => {
  ...
  return Math.max(0, Math.floor((Date.now() - createdOn.getTime()) / ONE_DAY));
};
...
export default function PartsDeliveryPage() {
  ...
  const [openPoDetailsByMaterial, setOpenPoDetailsByMaterial] = useState<Record<string, OpenPoDetail[]>>({});
  ...
  useEffect(() => {
    Promise.all([
      ...
      get(ref(getDatabase(), 'app_admin/openpo_vendor_3060_extra')),
    ]).then(([ticketSnap, allParts, openPoSnap, openPoExtraSnap]) => {
      setTickets((ticketSnap.val() || {}) as Record<string, Ticket>);
      setStockByMaterial(Object.entries(allParts || {}).reduce<Record<string, number>>((acc, [material, part]) => {
      ...
      const qtyMap = Object.values((openPoSnap.val() || {}) as Record<string, OpenPoItem>).reduce<Record<string, number>>((acc, item) => {
        ...
        const extraKey = makeExtraKey(po, poItem, part);
        const status: OpenPoDetail['status'] = extraMap[extraKey]?.actualShipmentDate ? 'intransit' : 'notshipped';
        const openQty = Number(item.openqty || 0);
        const ageingDays = daysSince(item.orderdate);

        acc[part] = (acc[part] || 0) + openQty; // include both notshipped + intransit
        detailMap[part] = detailMap[part] || [];
        detailMap[part].push({ poNumber: po, poItem, part, openQty, status, ageingDays });
        return acc;
      }, {});

      Object.values(detailMap).forEach((arr) => arr.sort((a, b) => b.ageingDays - a.ageingDays));
      setOpenPoByMaterial(qtyMap);
      setOpenPoDetailsByMaterial(detailMap);
    });
  }, []);
  ...
  const rows = useMemo(() => {
    ...
    Object.values(tickets).forEach((entry) => {
      ...
      if (status === 'closed' || rejectionStatus === 'Fully Rejected') return;
      const ageDays = getAgeDays(entry);
      parseSalesOrderDetails(ticketData['Sales Order Details']).filter((item) => item.deliveryCount === 0).forEach((item) => {
        if (!item.material) return;
        const key = `${item.material}__${item.description}`;
        const current = map.get(key) || { key, material: item.material, description: item.description, totalQty: 0, totalTickets: 0, oldestAging: 0, stockQty: 0, openPoQty: 0, details: [], openPoDetails: [] };
        current.totalQty += item.orderQty || 1;
        const tid = String(ticketData.TicketID || '-');
        if (!current.details.some((x) => x.id === tid)) current.details.push({ id: tid, ageDays, ticket: ticketData });
        ...
        current.openPoDetails = openPoDetailsByMaterial[item.material] || [];
        map.set(key, current);
      });
    });
    return Array.from(map.values()).sort((a, b) => b.oldestAging - a.oldestAging);
  }, [tickets, stockByMaterial, openPoByMaterial, openPoDetailsByMaterial]);
