...
export default function ManagerApprovalPage() {
  ...
  useEffect(() => {
    if (!application?.managerEmail) return;
    setSavedSignature(localStorage.getItem(`manager-signature:${application.managerEmail.toLowerCase()}`) || '');
  ...
  const approve = async () => {
    ...
    try {
      ...
      if (saveSignature && application.managerEmail) localStorage.setItem(`manager-signature:${application.managerEmail.toLowerCase()}`, signature);
      const updated = await FirebaseService.approvePartApplicationByManager(application.id, token, signature);
  ...
  const highlightedKeys = new Set(['supplier', 'supplierSapCode', 'supplierPartCode', 'standardPrice', 'wholesalePrice', 'retailPrice', 'newSupplier', 'newPrice', 'originalSupplier', 'originalPrice', 'originalWholesalePrice', 'originalRetailPrice', 'minimumOrderQuantity']);
  const isUseful = (value: any) => value !== '' && value != null && String(value).trim().toUpperCase() !== 'N/A';
  const highlights = Object.entries(application || {}).filter(([key, value]) => highlightedKeys.has(key) && isUseful(value));
  const details = Object.entries(application || {}).filter(([key, value]) => !hiddenFields.has(key) && !highlightedKeys.has(key) && isUseful(value) && !['managerApprovalRequired', 'status', 'managerName', 'managerEmail', 'partName'].includes(key));
  return <main className="min-h-screen bg-slate-50 py-8 px-4"><div className="mx-auto max-w-4xl space-y-6">
    <div className="rounded-xl bg-violet-700 p-6 text-white shadow-lg"><Badge className="mb-3 bg-white text-violet-700"><ShieldCheck className="mr-1 h-3 w-3" />Secure manager approval</Badge><h1 className="text-3xl font-extrabold tracking-tight">REVIEW AND SIGN</h1><p className="mt-2 text-violet-100">Please verify all application details, provide your electronic signature, and approve below.</p></div>
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><FileSignature className="h-5 w-5" />{application?.id}</CardTitle></CardHeader><CardContent className="space-y-5">{isUseful(application?.partName) && <div className="rounded-xl border-2 border-blue-300 bg-blue-50 p-5 shadow-sm"><p className="text-xs font-extrabold uppercase tracking-wider text-blue-700">Part Name</p><p className="mt-2 text-2xl font-extrabold text-blue-950">{application?.partName}</p></div>}{highlights.length > 0 && <div><h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-violet-800">Supplier and Pricing</h2><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{highlights.map(([key, value]) => <div key={key} className="rounded-lg border-2 border-violet-200 bg-violet-50 p-4"><p className="text-xs font-bold uppercase text-violet-700">{labelFor(key)}</p><p className="mt-1 text-lg font-extrabold text-violet-950">{displayValue(value)}</p></div>)}</div></div>}<div className="grid grid-cols-1 gap-x-8 md:grid-cols-2">{details.map(([key, value]) => <div key={key} className="border-b py-3"><p className="text-xs font-medium uppercase tracking-wide text-gray-500">{labelFor(key)}</p><p className="mt-1 whitespace-pre-wrap break-words text-sm">{displayValue(value)}</p></div>)}{application?.imageUrl && <div className="md:col-span-2 pt-4"><p className="mb-2 text-xs font-medium uppercase text-gray-500">Part Image</p><img src={application.imageUrl} alt="Part" className="max-h-72 rounded border object-contain" /></div>}</div></CardContent></Card>
    <Card><CardHeader><CardTitle>Electronic signature *</CardTitle></CardHeader><CardContent className="space-y-4">{savedSignature && <Button type="button" variant="outline" onClick={useSavedSignature}>Use My Saved Signature</Button>}<canvas ref={canvasRef} width={800} height={200} onPointerDown={startDrawing} onPointerMove={draw} onPointerUp={() => drawing.current = false} onPointerCancel={() => drawing.current = false} className="h-48 w-full touch-none rounded-lg border-2 border-dashed bg-white" aria-label="Signature pad" /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={saveSignature} onChange={(event) => setSaveSignature(event.target.checked)} className="h-4 w-4" />Save this signature on this device for my future approvals</label><div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><Button type="button" variant="outline" onClick={clear}><Eraser className="mr-2 h-4 w-4" />Clear</Button><Button type="button" onClick={approve} disabled={!hasSignature || submitting}>{submitting ? <LoadingSpinner size="sm" /> : <CheckCircle className="mr-2 h-4 w-4" />}Sign &amp; Approve</Button></div>{error && <p className="text-sm text-red-600">{error}</p>}<p className="text-xs text-gray-500">By selecting Sign &amp; Approve, you confirm that this electronic signature represents your approval of the application shown above.</p></CardContent></Card>
  </div></main>;
}
