...
export default function TruckAndChassisManagementPage() {
  ...
  const notIssuedCount = rows.filter((row) => !isIssued(row.isissuedtoproduction)).length;
  ...
  const filteredRows = useMemo(() => {
    const cardFilteredRows = rows.filter((row) => {
      ...
      if (cardFilter === 'notIssued') return !isIssued(row.isissuedtoproduction);
  ...
  useEffect(() => setPage(1), [search, cardFilter]);
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card
          ...
          onKeyDown={(event) => event.key === 'Enter' && setCardFilter('all')}
        ...
        <Card
          role="button"
          tabIndex={0}
          onClick={() => setCardFilter('issued')}
          onKeyDown={(event) => event.key === 'Enter' && setCardFilter('issued')}
        ...
        <Card
          role="button"
          tabIndex={0}
          onClick={() => setCardFilter('notIssued')}
          onKeyDown={(event) => event.key === 'Enter' && setCardFilter('notIssued')}
        ...
        <Card
          role="button"
          tabIndex={0}
          onClick={() => setCardFilter('description')}
          onKeyDown={(event) => event.key === 'Enter' && setCardFilter('description')}
