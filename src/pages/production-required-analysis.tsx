...
export default function ProductionRequiredAnalysisPage() {
  ...
  const [rows, setRows] = useState<Array<SummaryItem & { effSea: number; effNoSea: number; shortageSea: number; shortageNoSea: number; openPos: OpenPoItem[]; normPart: string; openPoQtyTotal: number; chassisCount: number; chassisDetails: Array<{ chassisNo: string; status: string; isSea: boolean }> }>>([]);
  const [search, setSearch] = useState('');
  const [mode, setMode] = useState<Mode>('nosea');
  const getPartDesc = (item: SummaryItem) =>
    resolvePartDescription(lang, {
      SPRAS_EN: item.spras_en || item.description,
  ...
  useEffect(() => {
    (async () => {
      ...
      summary.forEach((item) => {
        ...
        if (!grouped[part]) grouped[part] = { part, description: getPartDesc(item), spras_en: item.spras_en, spras_zh: item.spras_zh };
  ...
  return <div className="space-y-6">
    <h1 className="text-3xl font-bold">{mode === 'sea' ? (lang === 'zh' ? '潜在短缺零件' : 'Potential Shortage Parts') : (lang === 'zh' ? '关键短缺零件' : 'Critical Shortage Parts')}</h1>
